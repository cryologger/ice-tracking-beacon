/*
  Title:          Cryologger - Glacier Velocity (MGV)
  Author:         Adam Garbo
  Last modified:  June 26, 2018
  Description:    Glacier velocity measurement system deploymed on Milne Glacier, Ellesmere Island, Nunavut, Canada.
                  This code simply controls the power cycles of a ComNav GNSS, allowing it to record data for 1-hour a day.
*/
...
const float   resistor2     = 994000.0;     // Measured resistance (Ohms) of voltage divider R2 (MGV 1: 994 kOhm, MGV 2: 996 kOhm)
...
const float   minVoltage    = 11.0;         // Minimum measured battery voltage required to begin recording of GNSS observations

// Global variable declarations
byte            blinkLed            = 0;        // Flag for LED blink pattern
...
void setup()
{
  ...
  readBattery();    // Measure battery and select LED blink pattern
...
void loop()
{
  if ((alarmIsrWasCalled) || (wdtIsrWasCalled))
  {
    ...
    if (RTC.alarm(ALARM_1))   // Check alarm 1 and clear flag if set
    {
      ...
      readBattery();                            // Measure battery voltage and select LED blink pattern
      if (batteryVoltage > minVoltage)
      {
        digitalWrite(STEP_DOWN_EN_PIN, HIGH);   // Enable step-down voltage converter
        blinkLed = 3;                           // LED blink pattern selection flag
      }
    ...
    // LED blink patterns
    if (blinkLed == 1)    // Sleep: sufficient voltage
      blink(1, 5);
    if (blinkLed == 2)    // Sleep: insufficient voltage
      blink(2, 75);
    if (blinkLed == 3)    // Awake: recording GNSS observations
      blink(3, 250);
...
#ifdef DEBUG
  Serial.print(F("Battery voltage (V): ")); Serial.println(batteryVoltage);
#endif // DEBUG

  if (batteryVoltage > minVoltage)
  {
#ifdef DEBUG
    Serial.println(F("Battery voltage sufficient."));
#endif // DEBUG
    blink(4, 150);    // Sufficient voltage LED indicator
    blinkLed = 1;
  }
  else
  {
...
#endif // DEBUG
    blink(4, 1000);   // Low voltage LED indicator
    blinkLed = 2;
  }
}
